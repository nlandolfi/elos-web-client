...
/*
I partially converted this to blackriday v2 before bailing for time considerations... - NCL 2/15/2022
...
func (mr *markdownRenderer) RenderNode(w io.Writer, node *blackfriday.Node, entering bool) blackfriday.WalkStatus {
	log.Printf("formatting %s", node.Type)
...
func (mr *markdownRenderer) List(out *bytes.Buffer, text func() bool, flags blackfriday.ListType) {
	marker := out.Len()
	doubleSpace(out)
...
func (mr *markdownRenderer) ListItem(out *bytes.Buffer, text []byte, flags blackfriday.ListType) {
	if flags&blackfriday.ListTypeOrdered != 0 {
	...
	if mr.paragraph[mr.listDepth] {
		if flags&blackfriday.ListItemEndOfList == 0 {
			out.WriteString("\n")
		}
